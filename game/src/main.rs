...
fn main() {
    ...
    if let Some(x) = args.optional("--color_scheme") {
        ...
        for c in crate::colors::ColorSchemeChoice::choices() {
    ...
    if let Some(x) = args.optional("--challenge") {
        ...
        'OUTER: for (_, stages) in crate::challenges::Challenge::all() {
    ...
    if let Some(n) = args.optional_parse("--tutorial", |s| s.parse::<usize>()) {
        mode = Some(crate::sandbox::GameplayMode::Tutorial(
            crate::sandbox::TutorialPointer::new(n - 1, 0),
        ));
    }
    if mode.is_none() && flags.sim_flags.load.contains("scenarios/") {
        ...
        mode = Some(crate::sandbox::GameplayMode::PlayScenario(
            map_path,
            scenario,
            Vec::new(),
        ));
    ...
    widgetry::run(settings, |ctx| {
        crate::game::Game::new(flags, opts, start_with_edits, mode, ctx)
    });
}
